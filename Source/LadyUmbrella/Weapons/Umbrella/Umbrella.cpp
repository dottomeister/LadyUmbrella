...
void AUmbrella::Tick(float DeltaTime)
{
	...
	(void) OnUmbrellaStateChanged.ExecuteIfBound(IsAiming(), IsOpen(), IsOnTarget(),IsOnTargetGrapple());
	...
	(void) OnShootingCadence.ExecuteIfBound(ShootingTimer);
}
...
void AUmbrella::OnMontageNotifyBegin(FName NotifyName, const FBranchingPointNotifyPayload& BranchingPointPayload)
{
	...
	{
		ResetShooting();
		(void) OnCanShoot.ExecuteIfBound(bIsShooting, GetShootingCadence());
...
bool AUmbrella::OnElectricShoot()
{
	...
	World->SpawnActor<AElectricProjectile>(ElectricProjectileClass, SpawnLocation, ProjectileDirection.Rotation(), SpawnParameters);
...
void AUmbrella::SpawnGadgetCycleParticle() const
{
	...
	case EGadgetType::Grapple:
		...
		break;
	case EGadgetType::Electric:
		ParticleSpawnerComponent->SpawnParticleByName(EParticleEffectType::DartEquip, ParticleLocation, FRotator::ZeroRotator);
		break;
	default: break;
	}
}
