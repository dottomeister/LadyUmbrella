...
void UDescriptionBlock::SynchronizeProperties()
{
	...
	{
		ImageContainer->SetVisibility(bHasImage || bShowPlaceHolder ? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	}
	if (IsValid(DescriptiveImage))
	{
		DescriptiveImage->SetVisibility(bHasImage ? ESlateVisibility::Visible : ESlateVisibility::Hidden);
	}
}
