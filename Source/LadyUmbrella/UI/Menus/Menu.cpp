...
TArray<UUserWidget*>& UMenu::GetOptions()
{
	...
	for (UWidget* Child : MenuInputContainer->GetAllChildren())
	{
		...
		if (Child->IsA(UAnimatedBaseButton::StaticClass()) || Child->IsA(UInputElement::StaticClass()) || Child->IsA(UUpgradeSlot::StaticClass()))
		{
...
int UMenu::GetNextProbable(bool bPrevious, int& OriginalTarget, int& PredictedIndex)
{
	...
	{
		if (Result->GetIsEnabled() || PredictedIndex == OriginalTarget)
		{
