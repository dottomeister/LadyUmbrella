...
void UPlayerHealth::Show()
{
	if (APlayerCharacter* MainCharacter = Cast<APlayerCharacter>(GetOwningPlayer()->GetCharacter()))
	{
		...
		{
			HealthComponent->OnHealthAmountChanged.AddDynamic(this, &UPlayerHealth::UpdatePlayerHealthWithDelegate);
		}
	}
...
void UPlayerHealth::Hide()
{
	if (APlayerCharacter* MainCharacter = Cast<APlayerCharacter>(GetOwningPlayer()->GetCharacter()))
	{
		...
		if (IsValid(HealthComponent))
		{
			HealthComponent->OnHealthAmountChanged.RemoveDynamic(this, &UPlayerHealth::UpdatePlayerHealthWithDelegate);
		}
	}
